  ...
  class Account < Ibanity::BaseResource
    def self.list(financial_institution_id: nil, customer_access_token:, headers: nil, **query_params)
      uri = if financial_institution_id
        Ibanity.api_schema[:xs2a]["customer"]["financialInstitution"]["accounts"].sub("{financialInstitutionId}", financial_institution_id).sub("{accountId}", "")
      else
        Ibanity.api_schema[:xs2a]["customer"]["accounts"].sub("{accountId}", "")
      end
    ...
    def self.find(id:, financial_institution_id:, customer_access_token:, headers: nil)
      uri = Ibanity.api_schema[:xs2a]["customer"]["financialInstitution"]["accounts"]
